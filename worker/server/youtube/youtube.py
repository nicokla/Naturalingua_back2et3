...
from server.utils.utils import save_object, get_object, createNecessaryFolders
...
from server.youtube.youtube_api import YoutubeAPI
import os
from dotenv import load_dotenv
load_dotenv('/opt/app/server/youtube/.env')
...
from server.transliterateHehe.transliterateList import transliterateList
from server.transliterateHehe.languageCodes import languageToCodes
from server.transliterateHehe.postProcessing import postProcessing
...
# https://testdriven.io/blog/flask-async/
# async
def absorbYoutubeVideo(videoId, languageCodes, languageCodesKnown, alphabetId, sessionId, logger=print):
	fileName= f'/home/flask/mytmp/{sessionId}/{videoId}.txt'
	file1=open(fileName, 'w+')
	try:
		transcriptList = YouTubeTranscriptApi.list_transcripts(videoId)
	except Exception as e:
		# logger('%s no subs' % videoId)
		file1.close()
		return fileName
	for languageCode in languageCodesKnown:
		englishOk, transcriptAnglaisFetchable = getManualSub(transcriptList, languageCode)
		# logger('languageCode: %s / japaneseOk: %s' % (languageCode, japaneseOk))
		if englishOk:
			break
	...
	if(englishOk and japaneseOk):
		try:
			...
			# logger('%s erreur bizarre' % videoId)
			file1.close()
			return fileName
	else:
		# logger('%s snif' % videoId)
		return fileName
	# logger('%s youpi' % videoId)
...
# async
# def absorbYoutubeVideos(videoIds, language, sessionId):
# 	print('absorbYoutubeVideos',language)
# 	languageCodes=languageToCodes[language]
# 	aaa=0
# 	# root='/Users/nicolas/Desktop/tmp'
# 	root='/home/flask/mytmp'
# 	createNecessaryFolders(f'{root}/{sessionId}/{videoIds[0]}.txt')
# 	for videoId in videoIds:
# 		absorbYoutubeVideo(videoId, languageCodes, sessionId) # await
...
# language='french'
# sessionId='sessionId'
# absorbYoutubeVideos(videoIds, language, sessionId)
