...
def getHtml(fileName, title, isYoutube=True):
	...
	for index, line in enumerate(lines):
		...
		if(i == -15):
			continue
		content = ''
		canAddTime = (numJap % 10 == 0) and shouldHaveSpace
...
def createPdf(videoId, fileName, language, alphabetId, isYoutube=True):
	print('createPdf')
	...
	return html.write_pdf(stylesheets=[css], font_config=font_config) # fileName+'.pdf',


# videoIds=['E3Blxs0Wfco']
# videoId = videoIds[0]
# sessionId='sessionId'
# createPdf(sessionId, videoId)


# def absorbeEtCreeLesPdfs(videoIds, language, sessionId):
	# l = []
	# absorbYoutubeVideos(videoIds, language, sessionId)
	# for videoId in videoIds:
	# 	l.append(createPdf(sessionId, videoId))
	# return l
